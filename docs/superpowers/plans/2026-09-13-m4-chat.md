...
frontend/                              Tasks 10–14 (frontend half) own every detail below; files from spec §4
  ...
  src/api/queries.ts                   modify: useChatHistory(paperId), usePromoteNote()
  ...
  src/features/chat/promote.ts (+ promote.test.ts)       create: selection → {body, chunk_ids}
...
```bash
git add backend/app/models backend/alembic/versions/0003_chat_outputs.py backend/tests/test_llm_outputs.py
...
```bash
git add backend/app/core/papers.py backend/app/workers backend/tests/test_ingest.py
...
```bash
git add backend/prompts backend/app/core/prompts.py backend/app/core/errors.py backend/app/core/chat.py backend/tests/test_chat.py
...
```bash
git add backend/evals backend/tests/test_eval.py
...
- **The `paperId` fixture needs no change.** It already polls until `ready`. With Task 4's status flow (`chunking → embedding → ready`), `ready` means the fixture paper is embedded. If it weren't, chat would answer 409 `paper_not_indexed` and the specs would show that alert.
...
- `git status --short` shows only those two new files.
...
- Run `npx vitest run src/features/chat/citations.test.ts`.
...
/** Asks with Enter, and waits until the answer is saved: only a saved answer has its model footer. */
async function ask(page: Page, question: string) {
  await page.getByRole('textbox', { name: 'Question' }).fill(question)
  await page.getByRole('textbox', { name: 'Question' }).press('Enter')
  const answer = page.locator('article.chat-answer', { hasText: question })
...
function ProblemAlert({ paperId, problem, onRetry }: { paperId: string; problem: ChatProblem; onRetry: () => void }) {
  ...
  return (
    <Alert variant="destructive" className={cn('chat-error border-glass-border', glass)}>
...
|---|---|---|---|
| Sources wait for the whole answer | `ChatAnswer.tsx`: `{sources === null ? (` → `{sources === null \|\| !footer ? (` | `npx playwright test e2e/chat.spec.ts -g "streams in after its sources"` | `1 failed`: `Expected: true` / `Received: false` |
| An error wipes the partial answer | `useChatStream.ts`: in `fail`, `segments: appendSegments(s.segments, tail)` → `segments: []` | `npx playwright test e2e/chat.spec.ts -g "broken stream"` | `1 failed`: `Expected: "Partial answer [C"` / `Received: ""` |
| Saved answers never reach the history | `useChatStream.ts`: `return void invalidateHistory()` → `return` | `npx playwright test e2e/chat.spec.ts -g "reload with the paper"` | `1 failed` on `toHaveText` (only "Second question?" is listed) |
...
| Break | File and change | Run | Expected (captured) |
|---|---|---|---|
| Flash ignores zoom | `ReaderPage.tsx`: in the `.chunk-flash` div, `style={pdfRectToCss(rect, scale)}` → `style={pdfRectToCss(rect, 1)}` | `npx playwright test e2e/chat.spec.ts -g "clicking"` | `1 failed`: `Expected: < 2` / `Received: 219.66…` |
...
Run: `npx vitest run src/features/chat/promote.test.ts`
...
| Break | File and change | Run | Expected (captured) |
|---|---|---|---|
| Saving doesn't switch to Notes | `ReaderPage.tsx`: in `showPromotedNote`, delete `showTab('notes')` | `npx playwright test e2e/chat.spec.ts -g "saving a passage"` | `1 failed`: `Expected: "true"` / `Received: "false"` |
| Disabled button loses its reason | `SaveAsNoteButton.tsx`: `{canSave ? (` → `{true ? (` | `npx playwright test e2e/chat.spec.ts -g "no citation nearby"` | `1 failed`: `TimeoutError: locator.focus: Timeout 10000ms exceeded.` |
...
- **Board:**
  - Set M4 to `✅ Done (PR #<n>, merged <date>)`, with a link to this plan file, `docs/superpowers/plans/2026-09-13-m4-chat.md` (the file this section lives in).
