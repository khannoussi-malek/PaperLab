...
test("a number keeps the text the paper printed, unless the value or error is edited", async ({ page, request, tablePaperId }) => {
  // The fixture prints no suffixed numbers, so the candidates are faked; saving still goes to the real API.
  const candidates = [
    { raw: '110M', value: 110_000_000, error: null, unit_hint: 'parameters' },
    { raw: '34%', value: 34, error: null, unit_hint: null },
  ]
  await page.route('**/api/numbers/candidates', (route) => route.fulfill({ json: candidates }))
  const line = await openReader(page, tablePaperId, TABLE_LINE)
  const dialog = page.getByRole('dialog', { name: 'Add as number' })

  await selectSubstring(line, '88.5 ± 0.3 F1')
  await page.getByRole('button', { name: 'Add as number' }).click()
  await dialog.getByRole('radio', { name: '34%' }).click()
  await expect(dialog.getByRole('textbox', { name: 'Value' })).toHaveValue('34')
  await dialog.getByRole('textbox', { name: 'Label' }).fill('Share')
  await dialog.getByRole('button', { name: 'Add number' }).click()
  await expect(dialog).toBeHidden()

  await selectSubstring(line, '88.5 ± 0.3 F1')
  await page.getByRole('button', { name: 'Add as number' }).click()
  await expect(dialog.getByRole('textbox', { name: 'Value' })).toHaveValue('110000000')
  await dialog.getByRole('textbox', { name: 'Value' }).fill('120000000')
  await dialog.getByRole('textbox', { name: 'Label' }).fill('Parameters')
  await dialog.getByRole('button', { name: 'Add number' }).click()
  await expect(dialog).toBeHidden()

  await selectSubstring(line, '88.5 ± 0.3 F1')
  await page.getByRole('button', { name: 'Add as number' }).click()
  await dialog.getByRole('textbox', { name: 'Label' }).fill('Size')
  await dialog.getByRole('button', { name: 'Add number' }).click()
  await expect(dialog).toBeHidden()

  const [numbers] = await (await request.get(`/api/datasets?paper_id=${tablePaperId}`)).json()
  const dataset = await (await request.get(`/api/datasets/${numbers.id}`)).json()
  const values = dataset.rows.map((row: { cells: { raw: string; value: number | null }[] }) => row.cells.find((c) => c.value !== null))
  expect(values).toMatchObject([
    { raw: '34%', value: 34 },
    { raw: '120000000', value: 120_000_000 },
    { raw: '110M', value: 110_000_000 },
  ])
})
